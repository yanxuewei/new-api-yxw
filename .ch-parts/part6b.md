...
2. Web 应用防火墙 WAF 精确路径豁免（§8.3 坑 3）+ 应用侧 IP 白名单 + 签名校验三层。
3. 应用侧白名单配置示例：
...
**坑与注意事项**
- **坑 1｜只看 `X-Forwarded-For` 第一跳取客户端 IP。** 后果：攻击者伪造头即可绕过 IP 白名单 → **伪造回调完成支付**，属资金级漏洞。改进：应用型负载均衡 ALB / WAF 场景用**可信跳数**取 IP（`X-Forwarded-For` 从右往左取第 N 个，N=可信代理数）；或在 ALB 层注入 `X-Real-IP`，应用只信它。
...
| OSS | 标准+低频+归档+跨区复制 | | | | 低 |
| 全局流量管理 GTM Ultimate / DNS 付费版 | | 1 | | | 低 |
...
| 10 | 审计链路 | 操作审计 ActionTrail 投递 OSS/日志服务 SLS 且 ≥180 天可查（P1-17） | 查得到 90 天前的事件 |
...
| 14 | 镜像签名/来源 | 容器镜像服务 ACR 企业版 + 仅允许内网 VPC 域名拉取 + 禁 `latest` | 部署 manifest 全 SHA |
...
2. 容器水平伸缩（HPA）除 CPU 外加自定义指标（并发 SSE 连接数 / 队列深度），需 G8 的 `/metrics`；未落地前**只靠 CPU 会明显滞后**（SSE 是 IO 密集，CPU 很低但连接已满）→ 这是坑 1。
...
- 若走 **Web 应用防火墙 WAF 云原生接入（推荐）**：应用型负载均衡 ALB 入向**保持** `0.0.0.0/0:443`，把结论**书面记录**（为什么没按方案原文收敛），并补三条缓解：WAF 策略 + CC 阈值 + `alb-access` 日志服务 SLS 审计。
- 若走 **全站加速 DCDN 回源**：用 `aliyun dcdn DescribeDcdnL2Ips` 取回源段并同步进安全组；**每日定时任务比对差异并告警**（网段会变）。
...
**GUARD1（每档都要过，任一失败立即回滚）**

| 门 | 指标 | 阈值 | 数据来源 |
| --- | --- | --- | --- |
| G1-a | canary 5xx 率 | 不高于 stable 的 1.2 倍且绝对值 < 0.5% | ALB access log + 可观测监控 Prometheus 版 |
...
1. 通知窗口，冻结发布（§11.2）。
2. 云数据库 RDS 控制台 → 实例「基本信息」→ 左侧「服务可用性」→「切换为主备库」按钮（指定 5 分钟内执行），同时开始打流量：
...
- **坑 4｜切换演练把全局流量管理 GTM 判成不健康并切到新加坡。** 后果：跨区延迟突增 + 双活假象。改进：演练前**临时降低全局流量管理 GTM 敏感度或摘掉备池**（备池本就不该在 D7 前加入），并说明这属"人为制造的窗口"。
- **坑 5｜没测"RDS 完全不可用"分支（可用区整体故障需人工提工单）。** 改进：在 perf 环境用 `iptables` 模拟 DB 黑洞，验证应用是否 fail-fast 返回 503 而不是请求堆积打爆 FD —— 这条直接决定 §13 预案可行性。
...
| SLO | 指标 | 目标 | 窗口 |
| --- | --- | --- | --- |
| 可用性 | 外部拨测成功率（全局流量管理 GTM + 云监控站点监控 + blackbox 三源） | 99.95% | 月 |
...
# V3 看板数字与日志服务 SLS / 可观测监控 Prometheus 版原始查询一致（抽查 3 个面板）
```

**坑**
- **坑 1｜告警用邮箱。** 后果：夜里没人看。改进：P1 必须电话（云监控电话需余额/国际站可用性**先实测**；不可用则用应用实时监控服务 ARMS / 第三方 ONCALL 服务，并在 §12 记录实际通道）。
